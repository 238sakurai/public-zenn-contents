...
翌日たまたま、Cursor のデザインリード [Ryo Lu](https://x.com/ryolu_)さんが来日して開催された勉強会に参加しました。
...
ツールを1個足すだけでワークフローがぐっと楽になる感覚は、pup のときと同じです。一方で Prisma Plugin のくだりで感じたように、「便利だから全部つなげちゃえ」は危ない。((書き込みが絡む操作は僕は特にやらかしがちなので...))

あと、今回の検証で要望を書くなら、前回は pup に `traces` コマンドが欲しいと書きましたが、今回は Slack Plugin に**ファイル添付**が欲しいですね。
